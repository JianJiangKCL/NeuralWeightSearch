...
'''
	adding groups for mobilenet and vgg
'''
class NwsConv(nn.Module):

	def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, gs=1, groups=1):
	...
	def forward(self, x, quantizer):  # ):#
		tobuy_c = rearrange(self.weight, ' o (split i) h w -> o i (split h w)',
		                    split=self.gs, h=self.kernel_size, w=self.kernel_size)

		bought_c, diff, ids = quantizer(tobuy_c)

		bought_c = rearrange(bought_c, 'o i (split h w) -> o (split i) h w',
		                     split=self.gs, h=self.kernel_size, w=self.kernel_size)

		self.diff = diff

		out = F.conv2d(x, bought_c, None, self.stride, self.padding, groups=self.groups)

		return out

	def straight_forward(self, x):
		out = F.conv2d(x, self.weight, None, self.stride, self.padding, groups=self.groups)
		return out
...
class Nwsxxx(NwsConv):
	def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, gs=1, groups=1):
		super(Nwsxxx, self).__init__(in_channels, out_channels, kernel_size=kernel_size,
		                            stride=stride, padding=padding, gs=gs, groups=groups)
	...
	def expand_emb(self, expand_ratio):
		self.n_new_emb = int(self.qtz.n_embed * expand_ratio)
		self.qtz.add_emb(self.n_new_emb)

	def expand_emb_copied(self, expand_ratio, unique_indices):
		# unique_emb [ n_emb, dim]
		self.n_new_emb = int(self.qtz.n_embed * expand_ratio)

		self.qtz.add_emb_copied(self.n_new_emb, unique_indices)


class Nws3x3(Nwsxxx):
	def __init__(self, n_emb, in_channels, out_channels, stride=1, padding=0, gs=1, groups=1):
		super(Nws3x3, self).__init__(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, gs=gs, groups=groups)
...
class Nws1x1(Nwsxxx):
	...
	def __init__(
			self, n_emb, in_channels, out_channels, stride=1, padding=0, gs=1, groups=1):
		super(Nws1x1, self).__init__(in_channels, out_channels, kernel_size=1, stride=stride, padding=padding, gs=gs, groups=groups)
...
class Nws7x7(Nwsxxx):
	def __init__(
			self, n_emb, in_channels, out_channels, stride=2, padding=3, gs=1, groups=1):
		super(Nws7x7, self).__init__(in_channels, out_channels, kernel_size=7, stride=stride, padding=padding, gs=gs, groups=groups)
