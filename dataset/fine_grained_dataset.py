...
def ds_train_loader(path, train_batch_size, num_workers=24, pin_memory=False, normalize=None):
    if normalize is None:
        normalize = transforms.Normalize(
            mean=IMAGENET_MEAN, std=IMAGENET_STD)
    if 'cropped' in path:
        logging.info('cropped dataset')
...
def ds_val_loader(path, val_batch_size, num_workers=24, pin_memory=False, normalize=None):
    if normalize is None:
        normalize = transforms.Normalize(
            mean=IMAGENET_MEAN, std=IMAGENET_STD)
    if 'cropped' in path:
