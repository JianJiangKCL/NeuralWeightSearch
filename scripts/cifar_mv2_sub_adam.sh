...
SEED=${SEEDS[0]}
SAVE_PATH='results/cifar_experiments/sub_mv2_adam'
...
for TASK_ID in `seq 0 19`;
  do
   ...
   CUDA_VISIBLE_DEVICES=$GPU_ID python main.py -c ${CONFIG} --dataset_path ${DATASET_PATH} --results_dir $SAVE_PATH --end_class 5 --pretrained_end_class ${PRETRAINED_CLASS}  --finetune ${FINETUNE} --task_id $TASK_ID --seed $SEED  --use_recon_codes ${USE_RECON_CODES} --wandb_mode offline --disable_tqdm -t optimizer=adam -t name=adam_sub_ciar -t arch=mobilenetv2
